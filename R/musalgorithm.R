...
#' Algorithm for mus alternative account for norm of W
#'
#' @param W Matrix of measurements.
#' @param y Response vector.
#' @param lambda Regularization parameter due to residual.
#' @param delta Regularization parameter due to measurement error.
#'
musalgorithm_alt <- function(W, y, lambda, delta){
  # We assume the first column of W is constants, i.e., intercept
  n <- dim(W)[1]
  p <- dim(W)[2]
  obj <- c(rep(1,p),rep(0,p))
  ...
  # First "score function" constraint
  mat[(2*p+1),1:p] <- matrix(0, nrow=1, ncol=p)
  #mat[(2*p+2):(3*p),1:p] <- t(t(matrix(-delta, nrow=(p-1), ncol=p)) * apply(W, 2, function(x){sqrt(sum(x^2))})) / sqrt(n)
  mat[(2*p+2):(3*p),1:p] <- matrix(-delta, nrow=(p-1), ncol=p)
  mat[(2*p+1):(3*p),(p+1):(2*p)] <- 1/n*(t(W)%*%W)

  # Second "score function" constraint
  mat[(3*p+1),1:p] <- matrix(0, nrow=1, ncol=p)
  #mat[(3*p+2):(4*p),1:p] <- t(t(matrix(-delta, nrow=(p-1), ncol=p)) * apply(W, 2, function(x){sqrt(sum(x^2))})) / sqrt(n)
  mat[(3*p+2):(4*p),1:p] <- matrix(-delta, nrow=(p-1), ncol=p)
  mat[(3*p+1):(4*p),(p+1):(2*p)] <- -1/n*(t(W)%*%W)
...
#' @param delta Regularization parameter due to measurement error.
#'
musalgorithm_nb <- function(W, U, y, lambda, delta, delta1){
  # We assume the first column of W is constants, i.e., intercept
  n <- dim(W)[1]
  p <- dim(W)[2]
  obj <- c(rep(1,p),rep(0,p))
  ...
  #mat[(2*p+2):(3*p),1:p] <- t(t(matrix(-delta, nrow=(p-1), ncol=p)) * apply(W, 2, function(x){sqrt(sum(x^2))})) / sqrt(n)
  mat[(2*p+2):(3*p),1:p] <- matrix(-delta, nrow=(p-1), ncol=p)
  mat[(2*p+1):(3*p),(p+1):(2*p)] <- 1/n*(t(W)%*% (apply(W, 2, function(x) x * U)))

  # Second "score function" constraint
  mat[(3*p+1),1:p] <- matrix(0, nrow=1, ncol=p)
  #mat[(3*p+2):(4*p),1:p] <- t(t(matrix(-delta, nrow=(p-1), ncol=p)) * apply(W, 2, function(x){sqrt(sum(x^2))})) / sqrt(n)
  mat[(3*p+2):(4*p),1:p] <- matrix(-delta, nrow=(p-1), ncol=p)
  mat[(3*p+1):(4*p),(p+1):(2*p)] <- -1/n*(t(W)%*% (apply(W, 2, function(x) x * U)))


  rhs <- rep(0,(4*p))
  rhs[(2*p+1)] <- 1/n*(t(W[,1]) %*% (U * y))
  rhs[(2*p+2):(3*p)] <- lambda + 1/n*(t(W[,-1]) %*% (U * y)) + matrix(delta1, nrow=(p-1), ncol = 1)
  rhs[(3*p+1)] <- -1/n*(t(W[,1])%*% (U * y))
  rhs[(3*p+2):(4*p)] <- lambda - 1/n*(t(W[,-1]) %*% (U * y)) + matrix(delta1, nrow=(p-1), ncol = 1)
  dir <- rep("<=",4*p)
  bounds <- list(lower=list(ind=1:(2*p), val=rep(-Inf,2*p)),
                 upper=list(ind=1:(2*p), val=rep(Inf,2*p)))
